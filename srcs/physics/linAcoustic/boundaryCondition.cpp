                    ...
                    const std::vector<double>& fluxCoeffs)
{
	...
	double p = coeffs[0]*sin(2*M_PI*coeffs[1]*t + coeffs[2])+coeffs[3];
    double alpha = -(edgeNormal[0]*field.u[1][indexJ]
                    + edgeNormal[1]*field.u[2][indexJ])
                    -  (p - field.u[0][indexJ])/(fluxCoeffs[0]*fluxCoeffs[1]);
    double beta = edgeNormal[1]*field.u[1][indexJ]
                    + edgeNormal[0]*field.u[2][indexJ];
